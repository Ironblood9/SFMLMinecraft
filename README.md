🪓 This project is a mini 2D Minecraft clone built with SFML 3.
The goal is to explore game engine logic, block-based world systems, and core gameplay mechanics using C++.


🔹 Current Features

🌍 World generation

🚶‍♂️ Player movement

⛏️ Mining animation

🧱 Block breaking and removal system

🔹 Planned Features

🏗️ Block placement (Building)

⚔️ Sword attack animation

👾 NPCs and enemies

💬 Basic inventory / crafting system (future)

<img width="802" height="605" alt="Ekran görüntüsü 2025-11-02 213553" src="https://github.com/user-attachments/assets/071c1bd5-3821-4c21-98ad-187352258afd" />

<img width="802" height="601" alt="Ekran görüntüsü 2025-11-02 213756" src="https://github.com/user-attachments/assets/b4a50140-7bb3-47fc-8d6d-6bc7ca8a8386" />
🧰 Technologies
C++

SFML 3

he project is still under active development.
Suggestions, issues, and pull requests are always welcome! 🙌

📌 Author: [Demirkan Çevik]
📅 Status: In Development
🔗 Engine: SFML 3 (C++)
